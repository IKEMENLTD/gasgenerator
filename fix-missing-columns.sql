...
-- 4. conversation_statesテーブルにuser_idカラムを追加（TEXT型として）
...
-- 7. generated_codesテーブルに必要なカラムを追加（user_idはTEXT型として）
...
-- 9. conversation_sessionsテーブルに必要なカラムを追加（user_idはTEXT型として）
...
-- 10. claude_usageテーブルのuser_idをTEXT型に変換（UUIDからTEXTへ）
-- まず新しいカラムを作成
...
-- 11. claude_usage_logsテーブルも同様に処理
ALTER TABLE claude_usage_logs 
ADD COLUMN IF NOT EXISTS user_id_text TEXT;

UPDATE claude_usage_logs 
SET user_id_text = user_id::TEXT 
WHERE user_id IS NOT NULL;
...
INSERT INTO generation_queue (
  ...
  'U1234567890abcdef',  -- LINE User ID形式
  'U1234567890abcdef',
  gen_random_uuid(),
